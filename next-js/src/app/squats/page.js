...
export default function SquatsPage() {
  ...
  const [targetReps] = useState(15); // Target reps for squats
  ...
  const checkStatus = async () => {
    try {
      const response = await fetch('http://localhost:8000/status/squats');
      ...
      if (data.running) {
        setStatus(`AI Squat Tracker is running! Reps: ${currentReps}/${targetReps}`);
      } else {
  ...
  const startTracker = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8000/start-exercise/squats', {
        method: 'POST'
      });
      const data = await response.json();
  ...
  const stopTracker = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8000/stop-exercise/squats', {
        method: 'POST'
      });
      const data = await response.json();
  ...
  const completeWorkout = async () => {
    try {
      ...
      const response = await fetch('http://localhost:8000/complete-exercise/squats', {
        method: 'POST'
      });
      const data = await response.json();
      ...
      {!isCompleted && (
        <div className="max-w-2xl bg-purple-50 p-6 rounded-xl">
          ...
          <div className="text-left text-gray-700 space-y-2">
            ...
            <div>• Position yourself facing the camera with full body visible</div>
            <div>• Make sure you have enough space to squat safely</div>
            <div>• Do {targetReps} squats for a complete workout</div>
